...
def build_active_dataloader(dataset_cfg, class_names, batch_size, dist, root_path=None, workers=4,
    ...
    if active_training is not None:
        ...
        if cfg.DATA_CONFIG.DATASET == 'WaymoDataset':
        ...
        elif cfg.DATA_CONFIG.DATASET == 'KittiDataset': # kitti cases
            ...
            labelled_set.lidar_paths, labelled_set.infos = \
                active_training[0], active_training[1]
            unlabelled_set.lidar_paths, unlabelled_set.infos = \
                active_training[2], active_training[3]

            """ Open-world Setting: re-construct the anchor size based on GT labels """
            if cfg.get("OPEN_WORLD", None) and cfg.ACTIVE_TRAIN.TRAIN_RESUME is True:
                size_dict = {cls_name:[] for cls_name in labelled_set.class_names}
                for info in labelled_set.infos:
                    for gt_name, gt_box in zip(info['gt_names'].tolist(), info['gt_boxes'].tolist()):
                        if gt_name in labelled_set.class_names:
                            size_dict[gt_name].append(gt_box[3:6])

                """ update model configuration with new estimated anchor sizes"""
                for anchor_config in cfg.MODEL.DENSE_HEAD.ANCHOR_GENERATOR_CONFIG:
                    if anchor_config['class_name'] not in cfg.OPEN_WORLD.KNOWN_CLASS_NAMES:
                        anchor_config['anchor_sizes'][0] = np.mean(size_dict[anchor_config['class_name']], axis=0).tolist()
        ...
        if cfg.DATA_CONFIG.DATASET == 'WaymoDataset':
            ...
            pairs = list(zip(dataset.lidar_paths, dataset.infos))
            random.shuffle(pairs)

            # """ Discard pretrained random points with partially labels """
            # if cfg.get("OPEN_WORLD", None) and cfg.ACTIVE_TRAIN.TRAIN_RESUME is True:
            #     labelled_set.lidar_paths, labelled_set.infos = zip(*pairs[:1])
            #     unlabelled_set.lidar_paths, unlabelled_set.infos = zip(*pairs[1:])
            # else:
            labelled_set.lidar_paths, labelled_set.infos = \
                zip(*pairs[:cfg.ACTIVE_TRAIN.PRE_TRAIN_SAMPLE_NUMS])
            unlabelled_set.lidar_paths, unlabelled_set.infos = \
    ...
    if len(unlabelled_set) != 0:
        dataloader_unlabelled = DataLoader(
            unlabelled_set, batch_size=1, pin_memory=True, num_workers=workers,
