...
class AnchorHeadTemplate(nn.Module):
    ...
    def get_cls_layer_loss(self, new_data=None, reduce=True):
        ...
        if cfg.get('OPEN_WORLD', None) and cfg.OPEN_WORLD.METHOD == 'reweight_bg' and not cfg.ACTIVE_TRAIN.TRAIN_RESUME:
            energy = -torch.log(torch.sum(torch.exp(cls_preds), dim=-1)).detach() #
            bg_mask = (cls_targets==0)
            new_bg_weights = []
            softmax_T = cfg.OPEN_WORLD.TEMPERATURE
            for batch_id in range(energy.shape[0]):
                # bg_energy_list.append(energy[batch_id][bg_mask[batch_id]])
                # bg_weights_sum_list.append(cls_weights[batch_id][bg_mask[batch_id]].sum())
                new_weight = torch.softmax(energy[batch_id][bg_mask[batch_id]] / softmax_T, dim=-1) * cls_weights[batch_id][bg_mask[batch_id]].sum()
                new_bg_weights.append(new_weight)
                cls_weights[batch_id][bg_mask[batch_id]] = new_weight
