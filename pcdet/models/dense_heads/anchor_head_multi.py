...
class SingleHead(BaseBEVBackbone):
    ...
    def forward(self, spatial_features_2d):
        ...
        if cfg.ACTIVE_TRAIN.METHOD == "rankfeat":
            print(cls_preds.size())
            N, H, W, C = cls_preds.size()
            feat1 = cls_preds.view(N, C, H * W)
            u, s, v = torch.linalg.svd(feat1, full_matrices=False)
            feat1 = feat1 - s[:, 0:1].unsqueeze(2) * u[:, :, 0:1].bmm(v[:, 0:1, :])
            feat1 = feat1.view(N, H, W, C)
            cls_preds = feat1
...
class AnchorHeadMulti(AnchorHeadTemplate):
    ...
    def forward(self, data_dict):
        ...
        if cfg.get('OPEN_WORLD',None) and cfg.OPEN_WORLD.ENABLE and cfg.OPEN_WORLD.METHOD == "energy":
            data_dict['energy'] = -torch.log(torch.sum(torch.exp(batch_cls_preds), dim=2))

        return data_dict

    def get_cls_layer_loss(self, reduce=True):
