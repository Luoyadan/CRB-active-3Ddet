...
class Detector3DTemplate(nn.Module):
    ...
    def post_processing(self, batch_dict):
        ...
        rcnn_cls_raw = batch_dict['rcnn_cls'].view(batch_size, 5, -1)  if rcnn_cls is not None else None
        ...
        for index in range(batch_size):
            ...
            # full logits
            if 'full_cls_scores' in batch_dict.keys():
            ...
            for cls_idx in range(len(bbox_classes)):
                ...
                if num_cls_bbox > 0:
                    ...
                    box_size = gt_box[cls_mask].shape[-1]
                    gt_cls_box = gt_box[cls_mask].reshape(1, -1, box_size)
                    box_idxs_of_pts = roiaware_pool3d_utils.points_in_boxes_gpu(sampled_points, gt_cls_box[:,:,:7]).long().squeeze(dim=0)
            ...
            box_size = final_boxes.shape[-1]
            pred_boxes_reshaped = final_boxes.reshape(1, -1, box_size)
            ...
            pred_box_idxs_of_pts = roiaware_pool3d_utils.points_in_boxes_gpu(sampled_points,
                                                                        pred_boxes_reshaped[:,:,:7]).long().squeeze(dim=0)
            ...
            record_dict = {
                ...
                'rpn_preds': batch_dict['rpn_preds'] if 'rpn_preds' in batch_dict.keys() else None,
                'num_bbox': num_bbox,
                ...
                'pred_logits': final_logits if 'full_cls_scores' in batch_dict.keys() else None,
            ...
            # if report_status == "open_world":
            #     """We need report KNOWN / UNK only"""
            ...
            #     # iou = boxes_iou3d_gpu(pred_boxes[:,:7], gt_boxes[:,:7])
            #     # obj_mask = ((iou>iou_th) == True).nonzero(as_tuple=True)[0]
            #
            #     unk_gt_mask = gt_classes>len(cfg.OPEN_WORLD.KNOWN_CLASS_NAMES)
            #     unk_gt_boxes = gt_boxes[unk_gt_mask]
            #     unk_gt_classes = gt_classes[unk_gt_mask]
            #     iou = boxes_iou3d_gpu(pred_boxes[:,:7], unk_gt_boxes[:,:7])
            #     obj_mask = ((iou>iou_th) == True).nonzero(as_tuple=True)[0]
            #     found_unk_obj_count = obj_mask.shape[0]
            #     unk_recall = found_unk_obj_count / unk_gt_classes.shape[0]

                # match_pred_classes = pred_classes[obj_mask]
                # match_gt_classes = gt_classes.repeat(obj_mask.shape[0],1)[iou[obj_mask]>0]
                # compare_pred_gt = torch.eq(match_pred_classes, match_gt_classes)
                # known_match_count = match_pred_classes.shape[0]
                # unk_match_count = obj_mask.shape[0]-1


            # total_pred_count = pred_classes.shape[0]
            # # total_gt_count = gt_classes.shape[0]
            # # TP_count = compare_pred_gt[compare_pred_gt==True].shape[0]
            # # FP_count = total_pred_count - TP_count
            # # FN_count = total_gt_count - TP_count
            #
            # pred_bg_count = total_pred_count - obj_mask.shape[0]
            # pred_obj_count = obj_mask.shape[0]
            #
            # precision = TP_count / (TP_count + FP_count)
            # recall = TP_count / (TP_count + FN_count)
            # def f1(precision, recall):
            #     return 2*precision*recall/(recall+precision)
            # all_f1 = f1(precision, recall)
            #



            if 'roi_energy' in batch_dict.keys():
                record_dict['roi_energy'] = batch_dict['roi_energy'][index][selected]
            if 'prob_obj' in batch_dict.keys():
                record_dict['prob_obj'] = batch_dict['prob_obj'][index][selected]

            if rcnn_cls_raw is not None:
                record_dict['rcnn_cls_raw'] = rcnn_cls_raw[index]
    ...
    def generate_recall_record(box_preds, recall_dict, batch_index, data_dict=None, thresh_list=None):
        ...
        if recall_dict.__len__() == 0:
            recall_dict = {'gt_all': 0, 'gt_known': 0, 'gt_unk': 0}
            for cur_thresh in thresh_list:
                recall_dict['roi_all_%s' % (str(cur_thresh))] = 0
                recall_dict['rcnn_all_%s' % (str(cur_thresh))] = 0
                if cfg.get('OPEN_WORLD', None):
                    recall_dict['roi_known_%s' % (str(cur_thresh))] = 0
                    recall_dict['rcnn_known_%s' % (str(cur_thresh))] = 0
                    recall_dict['roi_unk_%s' % (str(cur_thresh))] = 0
                    recall_dict['rcnn_unk_%s' % (str(cur_thresh))] = 0
                    recall_dict['bg_rcnn_count_%s' % (str(cur_thresh))] = 0
        ...
        cur_gt_classes = cur_gt[:,-1]
        if cfg.get('OPEN_WORLD', None):
            known_mask, unk_mask = cur_gt_classes<=len(cfg.OPEN_WORLD.KNOWN_CLASS_NAMES), cur_gt_classes>len(cfg.OPEN_WORLD.KNOWN_CLASS_NAMES)
            known_gt_classes, unk_gt_classes = cur_gt_classes[known_mask], cur_gt_classes[unk_mask]
            known_gt, unk_gt = cur_gt[known_mask],  cur_gt[unk_mask]

        def compute_recall(box_preds, cur_gt, recall_dict, thresh_list, gt_type="all"):
            ...
            for cur_thresh in thresh_list:
                if iou3d_rcnn.shape[0] == 0:
                    rcnn_recalled = 0
                    recall_dict['rcnn_{}_{}'.format(gt_type, str(cur_thresh))] += rcnn_recalled
                else:
                ...
                if rois is not None:
                    roi_recalled = (iou3d_roi.max(dim=0)[
                                        0] > cur_thresh).sum().item()
                    recall_dict['roi_{}_{}'.format(gt_type, str(cur_thresh))] += roi_recalled
                if cfg.get('OPEN_WORLD', None) and gt_type == "all":
                    recall_dict['bg_rcnn_count_{}'.format(str(cur_thresh))] += (box_preds.shape[0]-rcnn_recalled)
            if cfg.get('OPEN_WORLD', None):
                recall_dict['gt_{}'.format(gt_type)] += cur_gt.shape[0]

        if cur_gt.shape[0] > 0:
            compute_recall(box_preds, cur_gt, recall_dict, thresh_list)

        if cfg.get('OPEN_WORLD', None):
            if known_gt.shape[0] > 0:
                compute_recall(box_preds, known_gt, recall_dict, thresh_list, gt_type="known")
            if unk_gt.shape[0] > 0:
                compute_recall(box_preds, unk_gt, recall_dict, thresh_list, gt_type="unk")
        return recall_dict
