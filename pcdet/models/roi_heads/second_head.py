...
class SECONDHead(RoIHeadTemplate):
    def __init__(self, input_channels, model_cfg, num_class=1, **kwargs):
        ...
        if self.model_cfg.get('PROB_OBJECTNESS_HEAD', None) and self.model_cfg.PROB_OBJECTNESS_HEAD:
            hidden_dim = self.model_cfg.SHARED_FC[-1]
            self.objectness_bn = nn.BatchNorm1d(hidden_dim, affine=False)
            self.objectness_flatten = nn.Flatten(0, 1)
    ...
    def forward(self, batch_dict):
        ...
        if cfg.ACTIVE_TRAIN.METHOD == 'react':
            x = shared_features
            for l in self.iou_layers[:5]:
                x = l(x)
            threshold = torch.topk(torch.flatten(x), int(0.9*len(torch.flatten(x))))[0][-1] # 90% based on ReAct paper
            x[x < threshold] = threshold
            for l in self.iou_layers[5:]:
                x = l(x)
            rcnn_iou = x.transpose(1, 2).contiguous().squeeze(dim=1)
        else:
            rcnn_iou = self.iou_layers(shared_features).transpose(1, 2).contiguous().squeeze(dim=1)  # (B*N, 1)


        if self.model_cfg.get('PROB_OBJECTNESS_HEAD', None) and self.model_cfg.PROB_OBJECTNESS_HEAD:
            x = shared_features.view(batch_dict['batch_size'], -1, shared_features.shape[-2])
            out = self.objectness_flatten(x)
            out = self.objectness_bn(out).unflatten(0, x.shape[:2])
            objectiveness_out = out.norm(dim=-1)**2
        ...
        if not self.training:
            if self.model_cfg.get('SAMPLING_ROUND', None):
                ...
                for i in range(self.model_cfg.SAMPLING_ROUND - 1):
                    ...
                    rcnn_cls = self.iou_layers(shared_features).transpose(1,2).contiguous().squeeze(dim=1)  # (B, 1 or 2)
            ...
            if self.model_cfg.get('PROB_OBJECTNESS_HEAD',None) and self.model_cfg.PROB_OBJECTNESS_HEAD:
                batch_dict['prob_obj'] = torch.exp(-objectiveness_out)
        else:
            targets_dict['rcnn_cls'], batch_dict['rcnn_cls'] = rcnn_cls, rcnn_cls

            if self.model_cfg.get('PROB_OBJECTNESS_HEAD', None) and self.model_cfg.PROB_OBJECTNESS_HEAD:
                targets_dict['prob_obj_out'] = objectiveness_out
    ...
    def get_loss(self, tb_dict=None):
        ...
        if self.model_cfg.get('PROB_OBJECTNESS_HEAD', None) and self.model_cfg.PROB_OBJECTNESS_HEAD:
            prob_obj_out: object = self.forward_ret_dict['prob_obj_out'].view(-1)
            match_mask = (self.forward_ret_dict['rcnn_cls_labels'].view(-1)>0.5).type(torch.uint8)
            match_num = match_mask.unique(return_counts=True)[-1][-1]
            prob_loss = 0.001 * torch.sum(match_mask * prob_obj_out)/match_num
            tb_dict['prob_loss'] = prob_loss.item()
            rcnn_loss = rcnn_loss + prob_loss
        tb_dict['rcnn_loss'] = rcnn_loss.item()
        return rcnn_loss, tb_dict

    def get_box_iou_layer_loss(self, forward_ret_dict):
        loss_cfgs = self.model_cfg.LOSS_CONFIG
        rcnn_iou = forward_ret_dict['rcnn_iou'] if 'rcnn_iou' in forward_ret_dict.keys() else forward_ret_dict['rcnn_cls']
        ...
        if loss_cfgs.IOU_LOSS == 'BinaryCrossEntropy':
            if 'weights' in forward_ret_dict.keys():
                batch_loss_iou = nn.functional.binary_cross_entropy_with_logits(
                    rcnn_iou_flat,
                    rcnn_iou_labels.float(), reduction='none', pos_weight=forward_ret_dict['weights'].view(-1)
                )
            else:
                batch_loss_iou = nn.functional.binary_cross_entropy_with_logits(
                    rcnn_iou_flat,
