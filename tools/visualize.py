...
def repeat_eval_ckpt(model, test_loader, args, eval_output_dir, logger, ckpt_dir, dist_test=False):
    ...
    # tensorboard log
    """ Put your WANDB account information here to log data"""
    if cfg.LOCAL_RANK == 0:
        ...
        wandb.init(project=cfg.DATA_CONFIG._BASE_CONFIG_.split('/')[-1].split('.')[0] + '_vis_select-{}'.format(cfg.ACTIVE_TRAIN.SELECT_NUMS), entity='xxxx')
    ...
    while True:
        ...
        active_label_files = [file for file in active_label_files if int(file.split('.')[0].split('_')[-3]) < int(cur_epoch_id)-1]
        # for example, pretrain epoch 1-40, selection round 40
        # performance from 41 to 80 should correspond to box number counted at epoch 40
        # 80 > 40 but 80 not > 80, so 80 will correspond to box number counted at epoch 40.
