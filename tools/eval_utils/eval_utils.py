...
def statistics_info(cfg, ret_dict, metric, disp_dict):
    for cur_thresh in cfg.MODEL.POST_PROCESSING.RECALL_THRESH_LIST:
        metric['recall_roi_%s' % str(cur_thresh)] += ret_dict.get('roi_all_%s' % str(cur_thresh), 0)
        metric['recall_rcnn_%s' % str(cur_thresh)] += ret_dict.get('rcnn_all_%s' % str(cur_thresh), 0)
    metric['gt_num'] += ret_dict.get('gt_all', 0)
    ...
    if dataset.dataset_cfg['DATASET'] == 'KittiDataset':
        ...
        for type in avg_types:
            for difficulty in difficulty_levels:
                ...
                new_value = sum(new_value) / len(new_value) if len(new_value) != 0 else 0
                result_dict[new_key] = new_value
    elif dataset.dataset_cfg['DATASET'] == 'NuScenesDataset':
        pass
    # TODO: NuScenes
    else:
